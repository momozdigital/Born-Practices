...
(function() {
	...
	ProductGridItem.prototype.compileTemplate = function(data) {
		...
		itemHtml = itemHtml.replace(/{{itemPrice}}/g, buildPrice(data));
      	...
      	/* MOMOZ CAMBIOS JC*/
      	  // Add Stock
          itemHtml = itemHtml.replace(/{{itemStock}}/g, buildItemStock(data));	
          // Add AddProductSoldOut
          itemHtml = itemHtml.replace(/{{addProductSoldOut}}/g, buildProductSoldOut(data));
          // Add capturerProductSoldOut
          itemHtml = itemHtml.replace(/{{capturerProductSoldOut}}/g, buildCapturerProductSoldOut(data));
          // Add ScriptAddProduct
          itemHtml = itemHtml.replace(/{{scriptAddProduct}}/g, buildScriptAddProduct(data));
		...
		itemHtml = itemHtml.replace(/{{itemId}}/g, data.id);
      	...
      	/* MOMOZ CAMBIOS JC*/
      		itemHtml = itemHtml.replace(/{{itemIdVar}}/g, data['variants'][0].id);
      		itemHtml = itemHtml.replace(/{{data}}/g, JSON.stringify(data));
      	
      	var titleP = data.title;
      	if(titleP.length<32){
          titleP = titleP+"<br><br>";
      	}
      
		itemHtml = itemHtml.replace(/{{itemTitle}}/g, titleP);
	...
	function buildPrice(data) {
  	...
  	/*MOMOZ CAMBIO JC*/
  	  ...
  	  function buildItemStock(data){
        
        var elem = document.getElementById("cant-"+data.id);
        var cant = "";
		...
		var html = '<div class="remain"><p class="texto"><b>'+texto+'</b></p></div>';
        
        return html;
      }	
  
      function buildProductSoldOut(data){
		var html = '';       
         var dataProduct = '';
        var elem = document.getElementById("dataProduct-"+data.id);
        if(elem!=null && elem!=undefined){
        	dataProduct = document.getElementById("dataProduct-"+data.id).value;
        }
        
        if(soldOut){
          	//html += '<spam class="avisame BIS_trigger" data-product-data=\''+dataProduct+'\'>¡Avísame!</spam><span id="avisar-{{itemId}}" class="ver-producto ver-agotado BIS_trigger" style="cursor: pointer;" data-product-data=\''+dataProduct+'\'> ver producto </span>';
        	html += '<spam class="avisame">¡Avísame!</spam><span id="avisar-{{itemId}}" class="ver-producto ver-agotado" style="cursor: pointer;"> ver producto </span>';
        }
        else{
        	html += '<span id="agregar-{{itemId}}" class="ver-producto add-product-cart" style="cursor: pointer;"> ver producto </span>';
        }
  	  ...
  	  function buildCapturerProductSoldOut(data){
        ...
        if(soldOut){
          //html += '<a href="#" class="BIS_trigger" data-product-data=\'{"id":'+data.id+',"title":"'+data.title+',"variants":'+data.variants+'"}\'>¿Quieres que te notifiquemos<br>cuando esté disponible?</a><a id="clickCapturer" href="#" class="BIS_trigger" data-product-data=\'{"id":'+data.id+',"title":"'+data.title+',"variants":'+data.variants+'"}\'">....</a>';
          html += '<a href="#" class="BIS_trigger" data-product-data=\''+dataProduct+'\'>¿Quieres que te notifiquemos<br>cuando esté disponible?</a><a id="clickCapturer" href="#" class="BIS_trigger" data-product-data=\''+dataProduct+'\'>....</a>';
        }
        else{
        	html += '<a id="clickCapturerAdd-{{itemId}}" class="clickCapturerAdd hideClickCapturer">&nbsp;&nbsp;</a><a class="clickCapturerAdd">&nbsp;&nbsp;</a>';
        }
        return html;
        
      }

      function buildScriptAddProduct(data){
		var html = '';
        return html;
      }
	...
	function buildLabels(data) {
		...
		//Calculate Percent //MOMOZ:      
      	var percent = ''
        if (onSale) {
        	var calc = (data.compare_at_price_min - data.price_min)/data.compare_at_price_min;
          	var por = calc*100;
          	percent = "<br><spam class='sale boost-pfs-filter-label' style='margin-left: 0px;'>"+Math.round(por)+"% OFF</spam>"
        }
      
		// Build Labels
		return soldOutLabel + percent;
	}
